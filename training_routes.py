...
async def get_courses(payload: dict = Depends(require_auth)):
    return await asyncio.to_thread(_agent.get_all_courses)

@router.get("/course/{course_id}")
async def get_course(course_id: str, payload: dict = Depends(require_auth)):
    course = await asyncio.to_thread(_agent.get_course_with_questions, course_id)
    if not course:
...
async def get_my_progress(payload: dict = Depends(require_auth)):
    ...
    return await asyncio.to_thread(_agent.get_user_completions, email)
...
async def submit_quiz(course_id: str, answers: Dict[str, int] = Body(...), payload: dict = Depends(require_auth)):
    ...
    if not email:
        raise HTTPException(status_code=401, detail="Email not found in token")
    result = await asyncio.to_thread(_agent.submit_quiz, email, course_id, answers)
    if "error" in result:
...
async def get_compliance_report(payload: dict = Depends(require_auth)):
    ...
    report = await asyncio.to_thread(_agent.get_compliance_report)
    if "error" in report:
...
async def send_reminders(payload: dict = Depends(require_admin)):
    count = await asyncio.to_thread(_agent.send_reminders)
